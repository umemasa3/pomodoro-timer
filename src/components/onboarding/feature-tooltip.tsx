...
// プリセットされた機能ツールチップ
export const TimerTooltip: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <FeatureTooltip
    title="ポモドーロタイマー"
...
export const TaskTooltip: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <FeatureTooltip
    title="タスク管理"
...
export const StatisticsTooltip: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <FeatureTooltip
    title="統計・分析"
    ...
    if (currentTooltip < tooltips.length) {
      ...
      if (element) {
        ...
        setTimeout(() => {
          ...
          if (currentTooltip + 1 >= tooltips.length) {
            handleComplete();
          } else {
            showNextTooltip();
          }
